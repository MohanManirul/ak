...
class RegistrationFeeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('backend.pages.students.registration_fee.view_registration_fee' , $data);
    }
    ...
    public function getStudent(Request $request){
    	...
    	foreach ($allStudent as $key => $v) {
    		...
    		$html[$key]['tdsource'] .='<a class="btn btn-sm btn-'.$color.'" title="PaySlip" target="_blank" href="'.route("student.reg.fee.payslip").'?class_id='.$v->class_id.'&student_id='.$v->student_id.'">Pay Slip</a>';
    		$html[$key]['tdsource'] .='</td>';
    	}
    ...
    public function paySlip(Request $request){
    	...
    	$class_id = $request->class_id;
    	 ...
    	 //dd($data['details']);
        $pdf = PDF::loadView('backend.pages.students.registration_fee.registration_payslip_pdf' , $data);
        return $pdf->stream('student-regstration-card.pdf');
    }
}