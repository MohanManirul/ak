...
class MonthlyFeeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('backend.pages.students.monthly_fee.view_monthly_fee' , $data);
    }
    ...
    public function getStudent(Request $request){
    	...
    	foreach ($allStudent as $key => $v) {
    		...
    		$html[$key]['tdsource'] .='<a class="btn btn-sm btn-'.$color.'" title="PaySlip" target="_blank" href="'.route("student.monthly.fee.payslip").'?class_id='.$v->class_id.'&student_id='.$v->student_id.'&month='.$request->month.'">Pay Slip</a>';
    		$html[$key]['tdsource'] .='</td>';
    	}
    ...
    public function paySlip(Request $request){
    	...
    	$data['month'] = $request->month;
    	 ...
    	 //dd($data['details']);
        $pdf = PDF::loadView('backend.pages.students.monthly_fee.monthly_payslip_pdf' , $data);
        return $pdf->stream('student-monthly_fee.pdf');
    }
}